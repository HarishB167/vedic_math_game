...
class Multiplication():
	...
	def play_level_2(self):
		level_2 = LevelMultiplication2(self.gameDisplay, self.clock)
		level_2.start()
...
class LevelMultiplication2(Level):
	...
	def next_question(self):
		...
		## Here result will multiplication of ones place numbers
		self.result = (self.num1 % 10) * (self.num2 % 10)
		if hasattr(self, 'input_box'):
			self.input_box = InputBox(self.input_box.rect.x, self.input_box.rect.y, 96, 32, self.result, self.gameDisplay, True)
	...
	def show_question(self):
		...
		elements_list = [
							...
							# [f"{self.num1 + self.num2-10} | {(self.num1-10) * (self.num2-10)}", Colors.BLACK, txt_rect_base.y+220, -1, -1, 1],

							[f"{self.num1 + self.num2-10}", Colors.BLACK, y_offset+220, -1, center_of_box_x-10, -1],
		...
		for element in elements_list:
			if element[0] == "input_box":
				if not hasattr(self, "input_box"):
					# InputBox args - x, y, w, h, result, screen, active=False, text=''
					self.input_box = InputBox(element[5], element[3], 96, 32, self.result, self.gameDisplay, True)
				continue
			txt_surface = self.get_font(20).render(str(element[0]), True, element[1])
			txt_rect = txt_surface.get_rect()
			txt_rect.top = element[2]
			...
			if element[5] == 1:
				...
				# print(f"Setting left value : {element[3]} {element[0]}")
				txt_rect.left = element[3]
